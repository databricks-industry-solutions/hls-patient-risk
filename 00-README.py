...
# MAGIC 
# MAGIC Longitudinal health records contain a wealth of information regarding a patient's risk factors. For instance, standard machine learning techniques can be employed to examine the correlation between a patient's health history and specific outcomes, such as heart attacks (refer to [this review article](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/s12874-021-01472-x)). In this case, we utilize a patient's medical condition history, medications taken, and demographic information as inputs. We then analyze a patient's encounter history to identify individuals diagnosed with a particular condition (CHF in this example) and train a machine learning model to predict the risk of an adverse outcome (emergency room admission) within a specified timeframe.
# MAGIC 
# MAGIC In this solution accelerator, we assume that the patient data are already stored in an OMOP 5.3 CDM on Delta Lake. We use OHDSI's patient-level risk prediction methodology to train a classifier that predicts the risk.
...
# MAGIC 
# MAGIC In this solution accelerator, we cover:
# MAGIC 
# MAGIC   1. Ingesting simulated clinical data from 100,000 patients prepared in OMOP 5.3 CDM and creating an OMOP data schema.
# MAGIC   2. Creating cohorts and cohort attributes.
# MAGIC   3. Generating features based on a patient's history.
# MAGIC   4. Training a classifier to predict outcomes using AutoML.
...
# MAGIC 
# MAGIC For our prediction example, we allow all patients who have experienced the outcome - emergency room visits - to be allowed in the target cohort. 
# MAGIC 
...
# MAGIC %md
# MAGIC Copyright / License info of the notebook. Copyright Databricks, Inc. [2023].  The source in this notebook is provided subject to the [Databricks License](https://databricks.com/db-license-source).  All included or referenced third party libraries are subject to the licenses set forth below.
# MAGIC 
